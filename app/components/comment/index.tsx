...
const CommentBox = ({prodId}:any) => {
    ...
    useEffect(()=>{
        fetch(`https://${process.env.SECRET_KEY}/api/fetchComments`,{method:"POST",body:JSON.stringify({
            productId: pID
        })}).then((response)=>
            response.json()
        ).then((data)=>
    ...
    const addComment = async(values:any) => {
        ...
        const commentResponse = await fetch("https://${process.env.SECRET_KEY}/api/addComment",{method:"POST",body:JSON.stringify({
