...
const Products = () => {
    ...
    const setFilterClass = (e:any, type:any) => {
        ...
        for(let i = 0; i < othercheckbox.length; i++) {
            othercheckbox[i]?.classList.remove("bg-amber-300")
        }
        checkbox?.classList.add("bg-amber-300");
    }
            ...
            <div className="flex flex-row w-full align-center">
                {
                    productCategories.map((type, index) => (
                        <div key={index} id={`box-category-${type}`} className="box-category flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700 w-40 m-2" onClick={(e)=>setFilterClass(e, type)}>
                            <input checked id={`category-${type}`} type="radio" value={type} name="category" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 hidden" />
                            <label htmlFor={`category-${type}`} className="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{type}</label>
                        </div> 
                    ))
                }
            </div>
